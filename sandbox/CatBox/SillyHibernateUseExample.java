...
public class SillyHibernateUseExample {
	...
	public static void main(String[] args) {
		...
		aSillyHibernateUseExample.showCourses();
		//   aSillyHibernateUseExample.modifyUser();
		//aSillyHibernateUseExample.modifyUser3();
	...
	private void addNewUsers() {
		...
		broUser.setUname("Josh");
		broUser.setPword("Brown");

		User nateUser = new User();
		nateUser.setUname("bunghole");
		nateUser.setPword("dumbhead");
		...
		session.save(broUser);
		session.save(nateUser);
		transaction.commit();
		/*
		...
		System.out.println("anotherUser generated ID is: " + broUser.getId());
	...
	private void showCourses() {
		...
		/*
		 * execute a HQL query against the database.  HQL is NOT SQL.  It is object based.
		 * "select e from Emph as e where e.course_id='1'"
		 * select e from Emph as e order by e.course_id
		 */
		Query allCoursesQuery = session.createQuery("select e from Emph as e where e.emphasis='pro'");
				
		/*
		 * get a list of User instances based on what was found in the database tables.
		 */
		emph = allCoursesQuery.list();
		System.out.println("num: "+emph.size());
		/*
		 * iterate over each User instance returned by the query and found in the list.
		 */
		Iterator<Emph> iter = emph.iterator();;
		while(iter.hasNext()) {
			Emph element = iter.next();
			System.out.println(element.toString());
		//	System.out.println("num of phone numbers: "+element.getPhoneNumbers().size());
		}
		transaction.commit();
	...
	private void addCourse(int id, int courseCredits, String corCode, String corDes, String corNam, String emph){
	...
	private void modifyUser2(){
		...
		Query customUserQuery = session.createQuery("select u from User where u.uname='josh'");
		User broUser = (User)customUserQuery.uniqueResult();

		broUser.setUname("michael");

		session.merge(broUser);
		transaction.commit();
		showAllUsers();
	}

	private void modifyUser3(){
		Session session = HibernateUtilSingleton.getSessionFactory().getCurrentSession();
		Transaction transaction2 = session.beginTransaction();

		Query customUserQuery = session.createQuery("select u from User as u where u.uname='josh'");
		User broUser = (User)customUserQuery.uniqueResult();

		broUser.setUname("pete");
		broUser.setPword("1234");
		session.merge(broUser);
		transaction2.commit();
		showAllUsers();
	}
